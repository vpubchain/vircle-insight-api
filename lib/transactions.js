...
var bitcore = require('vircle-bitcore-lib');
  ...
  if (output.data)
  {
    ...
    while(offset < data_bytes.length)
    {
      ...
      if (type == 7) // DEV_FUND_CFWD
      {
        ...
        var hex = transformed.data.toString(); // Force string conversion
        var array = [];
        for (var i = 0; (i < hex.length && hex.substr(i, 2) !== '00'); i += 2) {
          array.push(parseInt(hex.substr(i, 2), 16));
        }

        transformed.v.push({name:"result",value:decodeURIComponent(escape(String.fromCharCode.apply(null, array)))});
        break;
      } else if(type > 125 && type < 1000) {
        var hex = transformed.data.toString();
        var array = [];
        ...
        transformed.v.push({name:"InvoiceInfo",value:decodeURIComponent(escape(String.fromCharCode.apply(null, array)))});
        // transformed.v.push({name:"InvoiceInfo",value:hex});
        break;
      } else {
